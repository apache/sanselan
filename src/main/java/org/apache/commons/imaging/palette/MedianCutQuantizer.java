...
public class MedianCutQuantizer {
    ...
    private class ColorGroup {
        ColorGroupCut cut = null;
        int palette_index = -1;

        final List<ColorCount> color_counts;
        int min_red = Integer.MAX_VALUE;
        int max_red = Integer.MIN_VALUE;
        int min_green = Integer.MAX_VALUE;
        int max_green = Integer.MIN_VALUE;
        int min_blue = Integer.MAX_VALUE;
        int max_blue = Integer.MIN_VALUE;
        int min_alpha = Integer.MAX_VALUE;
        int max_alpha = Integer.MIN_VALUE;

        final int alpha_diff;
        final int red_diff;
        final int green_diff;
        final int blue_diff;

        final int max_diff;
        final int diff_total;

        ColorGroup(final List<ColorCount> color_counts) throws ImageWriteException {
    ...
    public Map<Integer, ColorCount> groupColors1(BufferedImage image, int max, int mask) {
        Map<Integer, ColorCount> color_map = new HashMap<>();
    ...
    Map<Integer, ColorCount> groupColors(BufferedImage image) {
    ...
    public Palette process(BufferedImage image, int max_colors, boolean verbose) throws ImageWriteException {
        Map<Integer, ColorCount> color_map = groupColors(image);
        ...
        if (discrete_colors <= max_colors) {
            ...
            List<ColorCount> color_counts = new ArrayList<>(color_map.values());
        ...
        List<ColorGroup> color_groups = new ArrayList<>();
        ColorGroup root = new ColorGroup(new ArrayList<>(color_map.values()));
        {
            color_groups.add(root);

            final Comparator<ColorGroup> comparator = (cg1, cg2) -> {
    ...
    private void doCut(ColorGroup color_group, final int mode, final List<ColorGroup> color_groups) throws ImageWriteException {
        ...
        Comparator<ColorCount> comparator = (c1, c2) -> {
            switch (mode) {
                case ALPHA:
        ...
        {
            List<ColorCount> color_counts1 = new ArrayList<>(color_group.color_counts.subList(0, median_index + 1));
            List<ColorCount> color_counts2 = new ArrayList<>(color_group.color_counts.subList(median_index + 1, color_group.color_counts.size()));
    ...
    private static class ColorGroupCut {
        final ColorGroup less, more;
        final int mode, limit;

        ColorGroupCut(ColorGroup less, ColorGroup more, int mode,
                      int limit) {
            this.less = less;
            this.more = more;
            this.mode = mode;
        ...
        ColorGroup getColorGroup(int argb) {
            int value;
            switch (mode) {
                case ALPHA:
                    value = 0xff & (argb >> 24);
